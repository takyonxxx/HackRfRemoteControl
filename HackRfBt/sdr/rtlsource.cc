      ...
      _buffer_size(1024 * 128), _device(nullptr)
{
    ...
    if (0 < rtlsdr_get_device_count()) {
        if (rtlsdr_open(&_device, device_idx)) {
            ...
            qDebug()  << "Can not open RTL2832 USB device " << device_idx;
            throw err;
        }
    } else {
        ConfigError err; err << "Can not open RTL2832 USB device: No device with index "
                             << device_idx << " found.";
        qDebug()  << "Can not open RTL2832 USB device: No device with index "
                 << device_idx << " found.";
        throw err;
    }
