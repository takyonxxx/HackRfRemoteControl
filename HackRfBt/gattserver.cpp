...
void GattServer::startBleService()
{
    ...
    charRxData.setUuid(QBluetoothUuid::ServiceClassUuid::AudioSource);
    ...
    charTxData.setUuid(QBluetoothUuid::ServiceClassUuid::AudioSink);
...
//    QObject::connect(leController.data(), &QLowEnergyController::errorOccurred, this, &GattServer::errorOccurred);
    ...
    advertisingData.setLocalName("HackRf");
    ...
    while (leController->state() != QLowEnergyController::AdvertisingState) {
        auto statusText = "Attempting to start advertising...";
        emit sendInfo(statusText);
        leController->startAdvertising(QLowEnergyAdvertisingParameters(), advertisingData, advertisingData);
        QThread::msleep(2000);  // Add a delay to avoid tight loop
    }
...
void GattServer::readValue()
{
    ...
    QLowEnergyCharacteristic cCharacteristic = service->characteristic(QBluetoothUuid::ServiceClassUuid::AudioSink);
...
void GattServer::writeValue(const QByteArray &value)
{
    ...
    QLowEnergyCharacteristic cCharacteristic = service->characteristic(QBluetoothUuid::ServiceClassUuid::AudioSource);
    Q_ASSERT(cCharacteristic.isValid());

    // Buffer the data and send it in chunks
    const int chunkSize = 132;  // Adjust as needed
    int bytesRemaining = value.size();
    int offset = 0;

    while (bytesRemaining > 0) {
        int currentChunkSize = qMin(bytesRemaining, chunkSize);
        QByteArray chunk = value.mid(offset, currentChunkSize);
        service->writeCharacteristic(cCharacteristic, chunk);

        // Update total data sent
        totalDataSentBytes += chunk.size();

        offset += currentChunkSize;
        bytesRemaining -= currentChunkSize;
    }
//    // Convert total data sent to megabytes (MB)
//    auto totalDataSentMB = static_cast<double>(totalDataSentBytes) / (1024 * 1024);
//    qDebug() << "Total Data Sent: " << totalDataSentMB << "MB";
}
