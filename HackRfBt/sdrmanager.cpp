...
void SdrManager::start()
{
    ...
    if (gattServer)
    {
        ...
        QObject::connect(gattServer, &GattServer::connectionState, this, &SdrManager::onConnectionStatedChanged);
        QObject::connect(gattServer, &GattServer::dataReceived, this, &SdrManager::onDataReceived);
    ...
    QObject::connect(m_Demodulator, &DemodulatorCtrl::spectrumUpdated, this, &SdrManager::fftTimeout);
    QObject::connect(m_Demodulator, &DemodulatorCtrl::filterChanged, this, &SdrManager::onFilterChanged);
...
void SdrManager::onReceiverStarted()
{
    qDebug() << "ReceiverStarted";
}

void SdrManager::onReceiverStopped()
{
    qDebug() << "ReceiverStopped";
}

void SdrManager::fftTimeout()
{
...
void SdrManager::onFilterChanged()
{
    m_HiCutFreq     = m_Demodulator->filterUpper();
    m_LowCutFreq    = m_Demodulator->filterLower();
}

void SdrManager::createMessage(uint8_t msgId, uint8_t rw, QByteArray payload, QByteArray *result)
{
...
bool SdrManager::parseMessage(QByteArray *data, uint8_t &command, QByteArray &value, uint8_t &rw)
{
...
void SdrManager::onConnectionStatedChanged(bool state)
{
    if(state)
    {
...
void SdrManager::onDataReceived(QByteArray data)
{
    ...
    if(rw == mWrite)
    {
        ...
        case mSetDeMod:
        {
            if (value >= DemodulatorCtrl::DEMOD_AM && value <= DemodulatorCtrl::DEMOD_BPSK31)
            {
                ...
                qDebug() << "Current demod:" << enumToString(static_cast<SdrManager::Demod>(currentDemod));
            }
            break;
        }
        case mIncFreq:
...
void SdrManager::sendCommand(uint8_t command, uint8_t value)
{
...
void SdrManager::sendString(uint8_t command, const QString& value)
{
    QByteArray sendData;
    QByteArray bytedata;
...
void SdrManager::setAbort(bool newAbort)
{
    m_abort = newAbort;
}

void SdrManager::onInfoReceived(QString info)
{
    qDebug() << info;
}
